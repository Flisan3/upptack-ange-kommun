...
function initializeActivityCard(card) {
    ...
    card.addEventListener("click", (e) => {
        if (e.target.closest(".map-btn-card")) {
            return; // Låt knappen fungera normalt utan att expandera kortet
        }
        ...
        // Expandera / stäng det klickade kortet
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // OBS: category-card får inte tabindex längre, så tabb går till knappar/länkar inuti
    cards.forEach(card => {
        if (!card.hasAttribute("role")) card.setAttribute("role", "button");
        if (!card.hasAttribute("aria-expanded")) card.setAttribute("aria-expanded", "false");
    });
    ...
    cards.forEach(card => {
        ...
        card.addEventListener("click", () => {
            ...
            // Ta bort "active" och "inactive" klasser från alla kort
            cards.forEach(c => {
            ...
            // Markera det klickade kortet som aktivt
            card.classList.add("active");
            card.setAttribute("aria-expanded", "true");

            // Uppdatera result-sektionen med detaljerad information
            // Map-btn-card får tabindex="0" så att den alltid är tabb-bar
                result.innerHTML = `
            ...
            // Initialisera interaktionen på det nya aktivitetkortet
            const resultCard = result.querySelector('.activity-card');
            initializeActivityCard(resultCard);
        });

        // Tangentbordsnavigering för att aktivera korten
        card.addEventListener("keydown", (event) => {
            if (event.key === "Enter" || event.key === " ") {
                card.click();
            }
